...
- 与手机端共享 `servers-v2`、分组、token 和服务器测速语义；连接身份按地址+token 区分。
- 会话历史、实时 mux/host、审批/提问、目标、子代理、队列和模型/推理设置；模型级自定义思考档位由共享 DSH 模型元数据驱动。
- 文件列表、预览、下载、上传和工作台绑定目录。
- 统计抽屉同时展示 token 量趋势与费用趋势；另有公告/更新/反馈、主题和网关能力显示。
...
总览中的 DSH 可达性与手机端一致，优先使用 `/health.upstreamReachable`；`host.describe` 失败后退避重试，并在双实时通道恢复时再次探测。

桌面文件树使用与手机端相同的盘符/UNC 父级和安全根语义，并可切换网关声明的多个允许根。新网关直接返回条目完整路径；连接旧网关时由桌面端按当前目录安全拼接，切换服务器时清除旧路径。

桌面端不复制手机端的模型提供方编辑器，但会消费相同的 `session.models` 模型元数据。自定义模型发布 `reasoning.efforts` 后，桌面会在当前会话的模型菜单中显示对应档位，并通过 `session.selectModel.reasoningEffort` 选择；未发布元数据时保留兼容档位。
...
### 2026-08-26：消费自定义模型思考档位
- 需求：桌面端与手机端使用一致的模型级思考档位。
- 方案：桌面端继续读取 `session.models` 的 `reasoning.efforts`，不复制手机模型配置编辑器。
- 联动：共享 01-contracts 中的模型元数据和 `session.selectModel.reasoningEffort`。
- 验证：保留现有桌面思考菜单静态测试，随全量门禁执行。
- 未做：本次未修改桌面布局或独立设置页。
