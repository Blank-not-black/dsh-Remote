...
桌面反馈与手机端使用同一 `includeDiagnostics` 契约，复选框默认关闭；启用后由网关而非浏览器生成并脱敏兼容性快照。
...
- 需求：让桌面用户也能在报告 DSH 接口问题时提交可分析的证据。
- 方案：反馈弹窗加入默认未选中的诊断选项及隐私说明，提交既有反馈端点。
- 联动：共享手机端、网关的 `includeDiagnostics` 契约。
- 验证：静态跨端 UI 契约测试待执行。
- 未做：不自动上传、不在浏览器保存诊断。
