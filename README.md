...
本仓库 `mod` 分支基于上游 v0.6.9（重大重构版）继续开发。v0.6.9-mod 新增下表 1–7 项改动；v0.7.0-mod 在此基础上修复手机端语音输入无效与离线语音包下载 404，并做全仓瘦身（下表 8–10 项）。其余功能与上游保持一致，完整说明请参考原项目 README。
...
| 8 | **手机端语音输入修复** | 语音识别默认改用系统语音服务（兼容性最广），不再盲目优先离线识别；未安装离线语言包的机型不再「一点就报错」；系统网络识别服务缺失时自动切换到设备端识别重试一次 |
| 9 | **离线语音包下载修复** | 移除必返回 404 的 example.com 占位默认地址；离线包下载必须填写真实可下载的直链 zip，留空会提示——不再出现「下载失败 HTTP 404」 |
| 10 | **全仓瘦身优化** | 清理网关、前端与脚本中的死代码与重复实现（约 660 行），移除未使用的 @capacitor/camera 依赖；零新增依赖，发布形态不变 |
