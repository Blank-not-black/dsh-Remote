...
test('真实生命周期：DSH 多次重启不断手机通道，网关重启后恢复事件', async (t) => {
  ...
  // 文件传输（/fs）已移除：改用非文件机制证明网关重启后状态持久化——
  // 令牌文件仍有效（带 token 访问 /workbench 返回 200），且工作台绑定状态文件保留。
  const workbench = await fetch(`${gateway.base}/workbench`, { headers: authHeaders() })
  assert.equal(workbench.status, 200)
  assert.deepEqual(await workbench.json(), { bound: false, path: null, title: null })
